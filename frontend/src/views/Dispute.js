...
import { getDispute, getMaxJurors, preVote, updateDisputeStatus, vote } from "../utils";
...
export default function Dispute() {
    ...
    useEffect(async () => {
        ...
        console.log(!d.votes.length || d.votes.find((v) => v.account !== window.accountId))
        setDispute(d)
        setLoading(false)
    }, [])
        ...
        <div>
            {
                loading ? (
                    ...
                    <div className="m-8">
                        {
                            (!dispute.accused_proves && (dispute.accused == window.accountId) && (dispute.dispute_status == "Open")) && isUserCreated &&
                            <div className="flex justify-center">
                                <button onClick={openModalCreateDispute} className="uppercase py-2 px-4 rounded-lg bg-red-500 border-transparent text-white text-md mb-4">
                                    Agregar pruebas!!!
                                </button>
                                <CreateDisputeDialog isOpen={isOpenCreateDispute} closeModal={closeModalCreateDispute} openModal={openModalCreateDispute} disputeId={dispute.id} />
                            </div>
                        }

                        {
                            ((window.accountId != dispute.accused) && (window.accountId != dispute.applicant) && (!dispute.jury_members.length || dispute.jury_members.find((v) => v !== window.accountId)) && isUserCreated && (dispute.dispute_status == "Open")) &&
                            <div className="flex justify-center">
...

                        {
                            ...
                            (!dispute.votes.length || dispute.votes.find((v) => v.account !== window.accountId)) &&
                            (dispute.dispute_status == "Voting")) &&
                            <div className="flex justify-center">
                                <button onClick={openModalOpenVoting} className="uppercase py-2 px-4 rounded-lg bg-red-500 border-transparent text-white text-md mb-4">
                                    Votar
                                </button>
                                                ...
                                                <div className="w-auto inline-block max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                                                    ...
                                                    <div className="mt-4 flex justify-center">
                                                        <div className="mr-4">
                                                            ...
                                                            <input checked={voteInFavor} onChange={handleVoteInFavor} type="checkbox" className="checked:bg-[#27C0EF] w-full"></input>
                                                        </div>
                                                        ...
                                                        <div>
                                                            ...
                                                            <input checked={voteAgaints} onChange={handleVoteAgaints} type="checkbox" className="checked:bg-[#27C0EF] w-full"></input>
                                                        </div>
                                                    </div>
                                                    <div className="mt-4">
                                                        <button
                                                            ...
                                                            onClick={async () => {
                                                                if (!voteAgaints && !voteInFavor) {
                                                                    toast.error("Por favor, medite, analice y vote por algun lado.")
                                                                    return
                                                                }
